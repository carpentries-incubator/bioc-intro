...
We are going to illustrate join using a first table, `rna_mini`, which 
is just a subset of the original `rna` table.

```{r}
load(file = "data/rna_mini.rda")
rna_mini
```
...
The `annot1` table is exactly the same than `annot2` except that gene names are 
